            ...
            <div className="space-y-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-gray-600">Deep dive into your store performance</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {[
                        { title: 'Total Revenue', value: '$84,250', change: '+12.5%', icon: DollarSign, color: 'text-primary-600', bg: 'bg-primary-50' },
                        { title: 'Orders Placed', value: '1,254', change: '+8.2%', icon: Package, color: 'text-indigo-600', bg: 'bg-indigo-50' },
                        { title: 'Avg Order Value', value: '$67.18', change: '-2.4%', icon: BarChart3, color: 'text-purple-600', bg: 'bg-purple-50' },
                        { title: 'Gross Profit', value: '$32,140', change: '+15.1%', icon: TrendingUp, color: 'text-success-600', bg: 'bg-success-50' },
                    ].map((stat, idx) => (
                        <div key={idx} className="metric-card">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <span className={`text-xs font-bold ${stat.change.startsWith('+') ? 'text-success-600' : 'text-danger-600'}`}>
                                    {stat.change}
                                </span>
                            </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {mockChartData.slice(0, 10).map((row, idx) => (
                                    <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                                        <td className="px-6 py-4 text-sm text-gray-600">{format(new Date(row.date), 'MMM dd, yyyy')}</td>
                                        <td className="px-6 py-4 text-sm font-bold text-gray-900 text-right">${row.sales.toLocaleString()}</td>
                                        <td className="px-6 py-4 text-sm font-bold text-success-600 text-right">${row.profit.toLocaleString()}</td>
                                        <td className="px-6 py-4 text-sm text-gray-900 text-center">{row.orders}</td>
                                        <td className="px-6 py-4 text-sm text-gray-900 text-right">${row.aov.toFixed(2)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
